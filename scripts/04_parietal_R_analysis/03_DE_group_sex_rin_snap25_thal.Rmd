...
subtitle: "Differential Expression: group + sex + rin + snap25 + thal"
author: "Kennedi Todd"
date: "09/11/2023"
output:
  html_document:
...
model <- "/model_group_sex_rin_snap25_thal"
```
...
rin <- dge.filtered.norm$samples$RIN
c1qb <- dge.filtered.norm$samples$C1QB
thal <- factor(dge.filtered.norm$samples$thal)
snap25 <- dge.filtered.norm$samples$SNAP25
braak <- factor(dge.filtered.norm$samples$braak)

design <- model.matrix(~ 0 + group + sex + rin + snap25 + thal) 
design.names <- c("control","meningitis","male","rin","snap25","thal1","thal3",
                  "thal4","thal5")
#design.names <- c("control","meningitis","male","rin","snap25")
