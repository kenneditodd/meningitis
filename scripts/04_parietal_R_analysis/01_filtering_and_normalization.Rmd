...
```{r set_variables}
tissue <- "parietal"
folder <- paste0(tissue,"_same_as_meninges")
...
if(folder == "parietal_same_as_meninges") {
...
data <- cpm(dge$counts, log = TRUE)
names <- paste0(dge$samples$informative_name,"_",dge$samples$RIN)
mds <- plotMDS(data, labels = names, col = rin_colors, top = 100)
...
ggplot(toplot, aes(x = Dim1, y = Dim2, color = RIN, label = Sample)) + 
  geom_point(size = 3) +
  scale_color_gradientn(colors = c("red","yellow","lightblue","blue")) +
  theme_bw() +
  geom_text(aes(label = Sample), size=4, vjust = -1)
