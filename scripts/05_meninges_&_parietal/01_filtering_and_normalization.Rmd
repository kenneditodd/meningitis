...
# read in counts data
counts1 <- read.delim(
  paste0("../../featureCounts/meninges_sex_specific_counts_matrix.tsv"),
  header = TRUE,
  row.names = 1,
  sep = "\t")
counts2 <- read.delim(
  paste0("../../featureCounts/parietal_sex_specific_counts_matrix.tsv"),
  header = TRUE,
  row.names = 1,
  sep = "\t")
...
colnames(counts) <- str_match(meta$filename, "(Mayo_[0-9]+_[MP])_S[0-9]+")[,2]
...
# cleanup 
remove(counts, counts1, counts2, meta, genes)
```
...
```{r}
keep <- !colnames(dge) %in% c("Mayo_24_M","Mayo_24_P","Mayo_08_P","Mayo_08_M",
                              "Mayo_18_M", "Mayo_18_P")
dge <- dge[,keep]
dim(dge)
```
...
pdf(paste0(path,".pdf"), width = 20, height = 20)
...
meta <- dge$samples[,c("sex","tissue")]
meta$tissue <- factor(meta$tissue)
paletteLength <- 100
myColor <- colorRampPalette(c("blue", "skyblue", "white"))(paletteLength)
ann_colors = list(tissue = c(meninges="orange", parietal="cornflowerblue"),
...
# set colors and get data
data <- cpm(dge$counts, log = TRUE)
dge$samples$tissue <- factor(dge$samples$tissue)
tissue_color <- group_colors[dge$samples$tissue]
names <- dge$samples$sample
...
for (dim in dims) {
  ...
  for (numGenes in topGenes) {
    ...
    legend(
      "topleft",
      legend = unique(dge$samples$tissue),
      pch = 16,
      col = unique(tissue_color),
      cex = 1
    )
    ...
    path <- paste0("../../results/",folder,"/prefiltering_QC/MDS_raw_", 
                   "top_", numGenes, "_",
                   "dim", as.character(dim)[1], "&", as.character(dim[2]), "_colored_by_tissue")
    saveToPDF(paste0(path, ".pdf"), width = 8, height = 8)
  }
}
...
Remove genes that don't have at least 1 CPM in 40 samples.
```{r filter}
start <- dim(dge)[1]
...
# set colors
colors <- group_colors[dge$samples$tissue]
nsamples <- ncol(dge)
...
saveToPDF(paste0(path, ".pdf"), width = 20, height = 12)
