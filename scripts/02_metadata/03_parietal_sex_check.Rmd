...
sex.info$mayo_id <- rownames(sex.info)
sex.info <- dplyr::left_join(sex.info,
                             dge$samples[,c("mayo_id","sex")],
                             by = "mayo_id")
...
# ----------------------
mayo_id <- sex.info$mayo_id # change to the individual ID column 
sex.info$mayo_id <- NULL # clean up 
# our model 
...
samples_to_investigate <- subset(sex_check, mayo_id %in% check_samples$mayo_id)
...
sex.info$mayo_id <- sex_check$mayo_id
low_coverage_samples_to_investigate <- subset(sex.info, mayo_id %in% low_coverage_female_samples$mayo_id)
```
...
colnames(genes_counts)[2] <- "mayo_id"

df <- as.data.frame(dge$samples$mayo_id)
df$sex <- dge$samples$sex
colnames(df) <- c("mayo_id","sex")

data <- merge(genes_counts, df, by = "mayo_id")
...
```{r}
wrong_sex_samples <-
  data[data$mayo_id %in% samples_to_investigate$mayo, ]
wrong_sex_samples <- unique(wrong_sex_samples)
low_coverage_samples <-
  data[data$mayo_id %in% low_coverage_samples_to_investigate$mayo_id, ]
wrong_sex_samples <- unique(wrong_sex_samples)
...
## CPM
Check sex genes and MBP.
```{r}
# choose genes and subset data
sex.genes <- c("XIST", "EIF1AY", "KDM5D", "UTY", "DDX3Y", "RPS4Y1","MBP")
