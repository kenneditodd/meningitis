...
```{r}
keep <- !colnames(dge) %in% c("Mayo_24_M","Mayo_24_P")
dge <- dge[,keep]
dim(dge)
```
...
# set colors and get data
data <- cpm(dge$counts, log = TRUE)
group_color <- group_colors[factor(dge$samples$tissue)]
names <- dge$samples$filename
...
```{r filter}
keep.expr <- filterByExpr(dge, group = dge$samples$sex_group, min.count = 1)
start <- dim(dge)[1]
...
colnames(log2cpm.raw) <- dge$samples$filename
log2cpm.filtered <- cpm(dge.filtered, log = TRUE)
colnames(log2cpm.filtered) <- dge.filtered$samples$filename
log2cpm.norm <- cpm(dge.filtered.norm, log = TRUE)
colnames(log2cpm.norm) <- dge.filtered.norm$samples$filename
...
saveToPDF(paste0(path, ".pdf"), width = 14, height = 12)
