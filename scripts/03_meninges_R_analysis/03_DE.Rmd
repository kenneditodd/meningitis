...
```{r}
dge.filtered.norm <- 
  readRDS(paste0("../../rObjects/",folder,"_DGEList_cell_markers.rds"))
```
...
for (i in 1:length(files)) {
 ...
 # comparison name
 comparison <- gsub(paste0("../../results/", folder, model, "/"), "", files[i])
 comparison <- gsub("_FDRq_1.00.tsv","",comparison)
 
  # save
  write.table(x = up,
            file = paste0("../../results/", folder, model, "/metascape_input_", comparison,
                          "_upreg_", format(thresh, nsmall = 2), ".tsv"),
            quote = FALSE,
            row.names = FALSE,
            col.names = FALSE)
    write.table(x = down,
            file = paste0("../../results/", folder, model, "/metascape_input_", comparison,
                          "_downreg_", format(thresh, nsmall = 2), ".tsv"),
            quote = FALSE,
            row.names = FALSE,
            col.names = FALSE)
}
