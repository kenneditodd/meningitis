...
subtitle: "Differential Expression: sex_group + SNAP25"
author: "Kennedi Todd"
date: "09/08/2023"
output:
  html_document:
...
model <- "/model_sexMeningitis_vs_sexControl_with_SNAP25"
```
...
## Design matrix
```{r design_matrix}
sex_group <- interaction(dge.filtered.norm$samples$sex_group)
snap25 <- dge.filtered.norm$samples$SNAP25

design <- model.matrix(~ 0 + sex_group + snap25)
design.names <- c("female_control", "female_meningitis", "male_control", 
                  "male_meningitis", "snap25")
...
contrasts <- makeContrasts(
  contrasts = c("female_meningitis - female_control", "male_meningitis - male_control"),
  levels = design.names)
head(contrasts)
...
for (i in files) {
 ...
 if(num != 0) {
    ...
    # save
    path <- paste0("../../results/",folder, model,"/volcano_",comparison,
...
# Metascape input
```{r}
df <- read.table(paste0("../../results/", 
                        folder, "/", model,
                        "/meningitis_vs_control_FDRq_0.05.tsv"),
                 sep = "\t", 
                 header = TRUE)

up <- subset(df, logFC > 0)
up <- up$ID
down <- subset(df, logFC < 0)
down <- down$ID

write.table(as.data.frame(up), 
            paste0("../../results/",
                   folder, "/", model,
                   "/metascape_input_FDRq_0.05_upreg.tsv"),
            quote = FALSE, row.names = FALSE, col.names = FALSE)

write.table(as.data.frame(down), 
            paste0("../../results/",
                   folder,"/", model,
                   "/metascape_input_FDRq_0.05_downreg.tsv"),
            quote = FALSE, row.names = FALSE, col.names = FALSE)
```
